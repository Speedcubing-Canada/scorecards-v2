...
// Event icons appear only on the side WITHOUT QR codes, and only in the compact
// (horizontal) layout, where space is tight enough that the QR side must stay uncluttered.
// The vertical layout keeps icons on both panels.
...
// Sized so three large slips (4–5 events) still fit one page alongside the inter-slip gap.
...
// Gap between consecutive slips. One extra blank line (SLIP_LINE_H) beyond the base 18pt,
// so the cut point between slips is unmistakable.

...
// Scramble double-checking variant: extra 12% scramblerCheck column (same as judge), taken from result (52→40).
  ...
  return e(View, { style: [styles.card, { position: 'absolute', left: pos.left, top: pos.top, width: cardW, height: cardH }] },
    ...
    e(View, { style: styles.table },
      e(View, { style: styles.headerRow }, ...Object.entries(cols).map(([col, w]) => {
        // DC cards use the compact result suffix so the narrower 40% column doesn't wrap to 4 lines.
        const resultLabel = doubleCheck
          ? 'Résultat (DNF si <...)\nResult (DNF if <...)'
          : 'Résultat (DNF si n\'est pas inférieur à ...)\nResult (DNF if not under ...)';
        const labels = { scrambler: 'Mélangeur\nScrambler', scramblerCheck: 'Vérif.\nCheck', attempt: 'Essai\nAttempt', result: resultLabel, judge: 'Juge\nJudge', competitor: 'Compétiteur\nCompetitor' };
